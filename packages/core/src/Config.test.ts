...
describe("defineConfig", () => {
  ...
  it("accepts a plain object for vars and no stages", () => {
    const plain = Config.defineConfig({ vars: { PORT: "3000" } });

    expectTypeOf<Config.StageOf<typeof plain>>().toEqualTypeOf<string>();
    expect(Object.keys(Config.varsFor(plain, "anything"))).toEqual(["PORT"]);
  });

  it("evaluates vars for one stage and turns a plain string into a literal", () => {
    const vars = Config.varsFor(staged, "production");
  ...
  it("gives a config without vars an empty record", () => {
    const providersOnly = Config.defineConfig({ providers: [memoryProvider({})] });

    expect(Config.varsFor(providersOnly, "development")).toEqual({});
  });
});
