...
export default defineConfig({
  ...
  vars: ({ stage, op, value, derive, custom, fromEnv }) => ({
    ...
    // A pure function of other values. Envi resolves the inputs in the batch and never caches the
    // result. `password` is an input only, so `envi run` does not inject it.
    REPLICA_URL: derive(
      {
    ...
    // An effectful value with an input. `resolve` returns an `Effect` without requirements. A
    // `CustomFailure` shows a safe message, and `transient` allows an expired entry of the same
    // inputs. Envi hides the message of every other error.
    API_TOKEN: custom({
      id: "api-token",
      from: { clientSecret: op("app", "oauth", "client-secret") },
      resolve: ({ clientSecret }) =>
        exchangeToken(clientSecret).pipe(
          Effect.timeout("5 seconds"),
          Effect.retry({ times: 2 }),
          Effect.mapError(
            () => new CustomFailure({ message: "The token exchange failed.", transient: true }),
          ),
        ),
